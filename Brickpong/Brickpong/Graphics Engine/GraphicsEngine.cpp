...
glm::mat4 GraphicsEngine::ChangeZoomLevel(Window* window, glm::mat4 currentMatrix) {
    if (window->getMouseButton(GLFW_MOUSE_BUTTON_LEFT) == GLFW_RELEASE) {
        ...
        glm::dvec2 target = window->getCursorPositionInWorldSpace(45.0f, 10.0f, glm::vec2(0.0f, -5.0f));
...
glm::mat4 GraphicsEngine::ChangeToOrtographic() {
...
glm::mat4 GraphicsEngine::ChangeProjection() {
    if (_zoom) {
